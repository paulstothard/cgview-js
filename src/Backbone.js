...
class Backbone extends CGObject {
  ...
  /**
   * Return the backbone color used beneath a contig.
   * @param {Contig} contig - Contig whose backbone color is required.
   * @return {Color} The contig override or alternating backbone color.
   * @private
   */
  ...
  refreshThickness() {
    ...
    if (pixelsPerBp > 1 && this.visible) {
      ...
      const detailThickness = this.sequence.detailThickness(pixelsPerBp);
      this._bpThicknessAddition = Math.max(0, detailThickness - zoomedThicknessWithoutAddition);
    } else {
      this._bpThicknessAddition = 0;
    }
  }
